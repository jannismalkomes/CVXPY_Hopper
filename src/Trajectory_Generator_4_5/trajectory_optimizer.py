...
class HopperParameters:
    ...
    def __init__(self):
        ...
        self.r_initial = np.array([0, 0, 0.1])
...
class GFOLDTrajectoryGenerator:
    ...
    def solve_problem_4(self, r0: np.ndarray, rf_target: np.ndarray) -> Dict[str, Any]:
        """
    ...
    def create_parameterized_problem(self) -> cp.Problem:
        ...
        # Precompute zero placeholders for N time steps
        mu1_param = cp.Parameter(self.N, value=np.zeros(
            self.N), name='mu1')  # mu_1 * (1 + z1)
        mu2_param = cp.Parameter(self.N, value=np.zeros(
            self.N), name='mu2')  # mu_2 * (1 + z0)
        # Precompute zero placeholders for N time steps
        z0_param = cp.Parameter(self.N, value=np.zeros(
            self.N), name='z0')  # mu_1 * (1 + z1)
        z1_param = cp.Parameter(self.N, value=np.zeros(
            self.N), name='z1')  # mu_2 * (1 + z0)
        ...
        m_wet_log = cp.Parameter(value=np.log(
            m_wet), name='m_wet_log')           # Log of wet mass
        ...
        for n in range(N):
            # z0_term = m_wet - alpha * thrust_max * n * 0.1
            # z1_term = m_wet - alpha * thrust_min * n * 0.1
            ...
            constraints += [s[0, n] >= c1_param[n] - mu1_param[n] * z[0, n]]
            constraints += [s[0, n] <= c2_param[n] - mu2_param[n] * z[0, n]]
            constraints += [z[0, n] >= z0_param[n]]
            constraints += [z[0, n] <= z1_param[n]]
    ...
    def export_c_code(self, output_dir: str = "code_export") -> str:
        ...
        print("🧪 Running final test with realistic parameters...")

        # Import the generated Python module dynamically
        import importlib.util
        spec = importlib.util.spec_from_file_location(
            "cpg_module", f"{output_dir}/cpg_solver.py")
        cpg_module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(cpg_module)

        # Run your integrated test
        self.test_exported_code(cpg_module)

        print("🎯 GFOLD C export and runtime validation completed successfully!")
        return output_dir

    def test_exported_code(self, cpg_module):

        # ================================
        # 1️⃣ Define all runtime parameters
        # ================================
        N = self.N
        dt = self.dt                        # Time step [s]
        m_wet = self.params.m_wet
        r1 = self.params.r1
        r2 = self.params.r2
        alpha = self.params.alpha
        V_max = self.params.V_max
        G_max = self.params.G_max
        cos_p_cs = self.params.cos_p_cs

        # Initial/final positions and velocities
        r_initial = self.params.r_initial
        v_initial = self.params.v_initial
        r_target = self.params.r_target
        v_target = self.params.v_target

        # ================================
        # 2️⃣ Precompute dependent arrays
        # ================================
        alpha_dt = alpha * dt / 2.0
        mu1 = np.zeros(N)
        mu2 = np.zeros(N)
        z0 = np.zeros(N)
        z1 = np.zeros(N)
        ...
        # ================================
        # 3️⃣ Run the C solver via f-string
        # ================================
        output_dir = "code_export"

        os.system(f'cd {output_dir} && /usr/local/python/current/bin/python -c "import cpg_module; import numpy as np; print(\'🚀 C code GFOLD solver - FINAL TEST\'); print(\'=\' * 50); '
                  f'updated = cpg_module.cpg_updated(); '
                  f'params = cpg_module.cpg_params(); '
                  f'params.r_initial = np.array({r_initial.tolist()}); '
                  f'params.v_initial = np.array({v_initial.tolist()}); '
                  f'params.r_target  = np.array({r_target.tolist()}); '
                  f'params.v_target  = np.array({v_target.tolist()}); '
                  f'params.dt        = {dt}; '
                  f'params.v_max     = {V_max}; '
                  f'params.g_max     = {G_max}; '
                  f'params.alpha_dt  = {alpha_dt}; '
                  f'params.cos_p_cs  = {cos_p_cs}; '
                  f'params.c1        = np.array({c1.tolist()}); '
                  f'params.c2        = np.array({c2.tolist()}); '
                  f'params.mu1       = np.array({mu1.tolist()}); '
                  f'params.mu2       = np.array({mu2.tolist()}); '
                  f'params.z0        = np.array({z0.tolist()}); '
                  f'params.z1        = np.array({z1.tolist()}); '
                  f'updated.r_initial = True; updated.v_initial = True; updated.r_target = True; updated.v_target = True; '
                  f'updated.dt = True; updated.v_max = True; updated.g_max = True; updated.alpha_dt = True; updated.cos_p_cs = True; '
                  f'updated.c1 = True; updated.c2 = True; updated.mu1 = True; updated.mu2 = True; updated.z0 = True; updated.z1 = True; '
                  f'result = cpg_module.solve(updated, params); '
                  f'print(f\'   Status: {{result.cpg_info.status}} (0=optimal)\'); '
                  f'print(f\'   Iterations: {{result.cpg_info.iter}}\'); '
                  f'print(f\'   Solve time: {{result.cpg_info.time*1000:.2f}} ms\'); '
                  f'print(f\'   Objective value: {{result.cpg_info.obj_val:.6f}}\'); '
                  f'log_final_mass = result.cpg_info.obj_val; final_mass = np.exp(log_final_mass); fuel_consumed = {m_wet} - final_mass; '
                  f'print(f\'\\\\n🚀 MISSION RESULTS:\'); '
                  f'print(f\'   Initial mass: {m_wet:.2f} kg\'); '
                  f'print(f\'   Final mass: {{final_mass:.2f}} kg\'); '
                  f'print(f\'   Fuel consumed: {{fuel_consumed:.2f}} kg\'); '
                  f'print(f\'   Flight time: {N*dt:.1f} s\'); '
                  f'print(f\'\\\\n📊 PERFORMANCE COMPARISON:\'); '
                  f'print(f\'   Python CVXPY: ~400ms solve time\'); '
                  f'print(f\'   Generated C:  ~{{result.cpg_info.time*1000:.1f}}ms solve time\'); '
                  f'print(f\'   Speedup: ~{{400/(result.cpg_info.time*1000):.0f}}x faster!\'); '
                  f'print(f\'\\\\n🎯 SUCCESS: GFOLD exported to C with runtime parameters!\')"')
...
if __name__ == "__main__":
    ...
    try:
        ...
        results = generator.solve_problem_4(
            generator.params.r_initial, generator.params.r_target)
