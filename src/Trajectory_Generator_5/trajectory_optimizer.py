...
class HopperParameters:
    ...
    def __init__(self):
        ...
        # Path and smoothness penalty weights
        self.lambda_path = 1e-3
        self.lambda_smooth = 1e-4
...
class GFOLDTrajectoryGenerator:
    ...
    def solve_problem_4(self, tf: float, r0: np.ndarray, rf_target: np.ndarray) -> Dict[str, Any]:
        ...
        # Time step
        dt_param = tf / (self.N - 1)

        # Compute path and smoothness penalties using params
        lambda_path = self.params.lambda_path
        lambda_smooth = self.params.lambda_smooth

        # Path length penalty
        path_penalty = cp.sum(cp.hstack([
            cp.norm(x[0:3, n+1] - x[0:3, n])
            for n in range(self.N - 1)
        ])) / (self.N - 1)

        # Smoothness penalty (difference in control vectors)
        accel_smoothness = cp.sum(cp.hstack([
            cp.norm(u[:, n+1] - u[:, n])
            for n in range(self.N - 2)
        ])) / (self.N - 2)

        total_penalty = lambda_path * path_penalty + lambda_smooth * accel_smoothness

        # Objective: maximize final mass minus path/smoothness penalty
        objective = cp.Maximize(z[0, self.N-1] - total_penalty)
    ...
    def create_parameterized_problem(self) -> cp.Problem:
        ...
        lambda_path_param = cp.Parameter(
            value=1e-3, pos=True, name='lambda_path')
        lambda_smooth_param = cp.Parameter(
            value=1e-4, pos=True, name='lambda_smooth')
        ...
        # Path length penalty
        # --- Path and smoothness regularization --- #
        dt_param = tf_param / (self.N - 1)

        # Path length penalty (normalized)
        path_penalty = cp.sum(cp.hstack([
            cp.norm(x[0:3, n+1] - x[0:3, n])
            for n in range(self.N - 1)
        ])) / (self.N - 1)
        ...
        total_penalty = lambda_path_param * path_penalty + \
            lambda_smooth_param * accel_smoothness

        # Objective (maximize final mass, penalize long or oscillatory paths)
        objective = cp.Maximize(z[0, self.N-1] - total_penalty)
...
if __name__ == "__main__":
    ...
    try:
        ...
        generator.data_exporter.export_problem4_data(
            results, filename_prefix="gfold_problem5"
        )
