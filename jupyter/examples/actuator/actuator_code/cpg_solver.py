...
from actuator_code import cpg_module
...
def cpg_solve(prob, updated_params=None, **kwargs):
    ...
    if updated_params is None:
        updated_params = ["lamb_sm", "kappa", "A", "w", "u_min", "u_max", "u_prev"]
    ...
    par.lamb_sm = squeeze_scalar(param_dict["lamb_sm"].value)
    par.kappa = list(param_dict["kappa"].value.flatten(order="F"))
    par.A = list(param_dict["A"].value.flatten(order="F"))
    par.w = list(param_dict["w"].value.flatten(order="F"))
    par.u_min = list(param_dict["u_min"].value.flatten(order="F"))
    par.u_max = list(param_dict["u_max"].value.flatten(order="F"))
    par.u_prev = list(param_dict["u_prev"].value.flatten(order="F"))
    ...
    prob.var_dict['u'].save_value(np.array(res.cpg_prim.u).reshape((8,), order='F'))
    prob.var_dict['delta_u'].save_value(np.array(res.cpg_prim.delta_u).reshape((8,), order='F'))
    prob.constraints[0].save_dual_value(np.array(res.cpg_dual.d0).reshape((8,), order='F'))
    prob.constraints[1].save_dual_value(np.array(res.cpg_dual.d1).reshape((8,), order='F'))
    prob.constraints[2].save_dual_value(np.array(res.cpg_dual.d2).reshape((8,), order='F'))
